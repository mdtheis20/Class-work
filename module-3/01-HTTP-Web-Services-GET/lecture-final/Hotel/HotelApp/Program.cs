    ...
    class Program
    {
        ...
        private static void MenuSelection()
        {
            ...
            while (menuSelection != 0)
            {
                ...
                {
                    // Here I would prompt the user for hotel id
                    PrintHotel( GetHotel(1) );
                }
        ...
        {
            RestRequest request = new RestRequest(API_URL + $"hotels/{id}");     // URL = "http://localhost:3000/hotels/100"
            IRestResponse<Hotel> response = client.Get<Hotel>(request);
            return response.Data;
        }

        private static List<Review> GetReviews(int hotelId)
        {
            RestRequest request = new RestRequest(API_URL + $"reviews?hotelID={hotelId}");
            IRestResponse<List<Review>> response = client.Get<List<Review>>(request);
            return response.Data;
        }
